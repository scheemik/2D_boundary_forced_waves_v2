...
def plot_bot(dset, image_axes, data_slices, x_lims=None, y_lims=None, image_scales=(0,0), clim=None, even_scale=False, cmap='RdBu_r', axes=None, figkw={}, title=None, func=None, paramfile=None, plot_contours=True):
    ...
    # Plot
    plot = paxes.pcolormesh(xmesh, ymesh, data, cmap=cmap, zorder=2.5)
    if plot_contours:
        # Fix the off-by-one error
        xmesh = xmesh[1:,1:]
        ymesh = ymesh[1:,1:]
        plat = paxes.contour(xmesh, ymesh, data, 10, cmap='coolwarm', zorder=3.5)#'RdBu_r')
    ...
    cbar = plt.colorbar(plot, cax=caxes, orientation='horizontal',
        ticks=ticker.MaxNLocator(nbins=3), format=ticker.FuncFormatter(latex_exp)) #fmt))
